...
namespace CoffeeShopClassLibrary
{
	...
	public class Order
	{
		...
		public List<IMenuItem> Items { get => _Items; set => _Items = value; }
		...
		public Order(Customer customer, Address addr)
		{
			...
			this._Items = new List<IMenuItem>();
		...
		public String GetOrders()
		{
			...
			{
				IMenuItem[] ItemsArray = Items.ToArray();
				for (int i = 0; i < ItemsArray.Length; i++)
				{
					temp += ItemsArray[i].Description + " - $" + ItemsArray[i].BaseCost + "\n";
				}
			}
		...
		public string GetInfo()
		{
			...
			return string.Format("\nOrders:\nOrder Id: {0}\nCustomer Name : {1}\nOrder Time: {2}\nCost : ${3}\n" +
				...
				OrderTime.ToString("hh:mm tt"), Cost, DeliveryAddress.Street, DeliveryAddress.City,
				DeliveryAddress.Province,  DeliveryAddress.PostalCode, Status, GetOrders());
		}
	}
}